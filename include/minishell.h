...
/* Pipe state indeksleri için enum tanımları */
typedef enum e_pipe_index {
    PIPE_ACTIVE = 0,      // Aktif pipe sayısı
    PIPE_INPUT_FD = 1,    // Input file descriptor
    PIPE_OUTPUT_FD = 2,   // Output file descriptor
    PIPE_INPUT_COUNT = 3, // Input redirection sayısı
    PIPE_OUTPUT_COUNT = 4,// Output redirection sayısı
    PIPE_TOTAL = 5,       // Toplam pipe sayısı
    PIPE_HAS_INPUT = 6,   // Input redirection flag
    PIPE_HAS_OUTPUT = 7,  // Output redirection flag
    PIPE_REDIR_COMBO = 8, // Redirection + pipe kombinasyonu
    PIPE_RESERVED1 = 9,   // Rezerve alan (heredoc için kullanılıyor)
    PIPE_CHILD_COUNT = 10,// Child process sayısı
    PIPE_EXEC_STATE = 11, // Execution state
    PIPE_RESERVED2 = 12,  // Rezerve alan
    PIPE_STATE_SIZE = 13  // Toplam boyut
} t_pipe_index;
...
void    safe_close(int fd);              // Güvenli fd kapatma fonksiyonu
