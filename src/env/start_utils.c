...
/*   Updated: 2025/08/07 21:22:08 by sedemir          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	fill_env_pair(t_env *env, char **orig_env, int idx)
{
	...
	env->parsed_env[idx][1] = galloc(
			(string_length(orig_env[idx], '\0') - len + 1) * sizeof(char));
	if (!env->parsed_env[idx][0] || !env->parsed_env[idx][1])
		return ;
	duplicate_string(env->parsed_env[idx][0], orig_env[idx], 0, len);
	duplicate_string(env->parsed_env[idx][1], orig_env[idx],
		len + 1, string_length(orig_env[idx], '\0'));
}
